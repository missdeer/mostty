...
### 第二档:先量化再做(收益明确,工程量大)
- VT #2 — reader → 每 tab SPSC ring + `PostMessage` 替代 `SendMessage`(~250-400 行)
  - 注意:`ReadMsg` 当前是栈对象,必须改成拷贝进 ring;ring 满时 reader 阻塞,不能丢字节
  - 验证:`cat 50 MB log`,看 reader → UI round-trip 间隔 vs `nextSlice` 耗时
