# 增加 D3D12 渲染后端评估（研究视角）

> 本文件是 `renderer-backend-abstraction.md`（总纲）的 M3 深化。共享设施
> （Renderer 抽象、shader single-source 管线、字体栈双 device、frame-latency
> 对等表、DComp 桥接矩阵、测试矩阵）见总纲，本文只写 D3D12 特有骨架。
>
> 立项动机：**代价最小、shader / DComp / RDP 全部零改动**——四后端里学习
> 曲线最平的一档。相对 D3D11 唯一实质新增是 explicit fence + descriptor
> heap + PSO，是通往 D3D12 explicit 模型的教科书路径。

基于实际代码结构给出评估（renderer 入口 `src/win32/d3d11.zig:1` 共 1009 行，
子模块 `src/win32/d3d11/` 12 个文件共 5105 行——含 `glyph_worker.zig` 713
行异步栅化 worker 与 `gpu.zig` 605 行 GPU 资源工厂；shader
`src/win32/terminal.hlsl:1` 共 223 行；外部调用面 35 处，散落在
`mosttywindows.zig` / `win32/{render,tab_mgmt,window_geom}.zig` /
`win32/wnd/{ime,misc,lifecycle,paint,mouse}.zig`——**完整清单见总纲 §二**）。
...
### 5. Swapchain + DirectComposition + frame-latency waitable

**这是 D3D12 相比 Vulkan/GL 的最大优势**：DXGI swapchain 与 DirectComposition
的对接逻辑跟 D3D11 几乎一样——`CreateSwapChainForComposition` 接受
`ID3D12CommandQueue`（而非 D3D11 device），后续
`IDCompositionVisual::SetContent(swapchain)` 流程完全不变。
`src/win32/d3d11/swap_chain.zig` 179 行的整套 DComp 装配逻辑可以**结构性
复用**，无外观损失、无透明/模糊回归风险。

**同样复用的还有 frame-latency waitable**（`f259ea4` 落地的 3-buffer
swap chain + `IDXGISwapChain2::GetFrameLatencyWaitableObject`，见
`d3d11.zig:103-108`）：DXGI 层与 D3D12 无关，waitable handle 拿到后与
`ID3D12Fence` GPU 完成信号在 `paint.zig` 节流路径里合并——这是 D3D12 端
"两条同步线"设计的关键点（见总纲 §五 Frame-latency 对等表）。
...
### 7b. 异步栅化 worker × D3D12 上传

`glyph_worker.zig` 713 行的异步栅化线程（`aa25390` 落地）产 `RasterResult`，
主线程通过 `applyGlyphResult` 上传到 atlas。D3D12 端的对应路径：worker
线程产 CPU bitmap → 主渲染线程从 upload heap ring buffer 拿 slice →
`CopyTextureRegion` 到 atlas + resource barrier
`COPY_DEST → PIXEL_SHADER_RESOURCE`。Ring buffer slice 必须与 fence 联动，
否则和 GPU 当前帧数据竞争。这一点在 D3D11 端由驱动隐式处理，是 D3D12 端
新增复杂度的一部分。
...
- **值得做**，且是四后端里学习曲线最平的一档：
  1. **Shader / DComp / RDP 全部零改动**，唯一新增的是 D3D12 的三大骨架
     （explicit fence、descriptor heap、PSO），是通往 explicit 模型的教科
     书路径；
  2. **DXGI 与 DComp 层复用**是本项目最漂亮的对照——同一套 swapchain +
     waitable 装配代码，一头挂 D3D11、一头挂 D3D12，能直接量出"driver 隐式
     同步 vs 应用显式 fence"的行为差异；
  3. **相对 Vulkan 的独立价值**：D3D12 的 explicit pipeline 是"部分显式"
     （DXGI/DComp/RDP 全借用 D3D11 侧路径），Vulkan 是"完全显式"，两者的
     骨架量差与手感差是本项目主要研究产出之一。
- **允许失败的范围**：无。这是四后端里唯一"骨架跑通即研究成功"的一档——
  没有 DComp 桥接不确定性、没有 RDP 妥协、没有驱动兼容矩阵。
- **相对 GL 4.6 的独立价值**：GL 4.6 走 SPIR-V ingest 是"高层 API +
  跨源 shader"；D3D12 是"低层 API + 原生 HLSL"，是最保守也是最完整的
  Windows GPU 路径。三方对照才把 shader 单源管线（总纲 M1）的收益说清楚。

**建议里程碑**：M3 单阶段，无需拆分。DXGI + DComp + waitable 全复用，
fence 与 waitable 的双同步在 `paint.zig` 节流路径里合并这一步是全篇最关键
的代码点，值得单独一次 code review。M3 完成后即可支撑 M6 对比报告里的
"D3D11 vs D3D12 拖窗口手感 + PTY 压力 CPU 占用"两轴数据。
