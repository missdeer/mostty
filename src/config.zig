...
pub const RendererBackend = enum {
    ...
    pub fn usesDwmRedirection(self: RendererBackend) bool {
        return self == .@"pure-opengl" or self == .@"native-vulkan";
    }
};
    ...
    while (it.next()) |raw_line| {
        ...
        if (std.mem.eql(u8, key, "theme")) {
            ...
            renderer = std.meta.stringToEnum(RendererBackend, value) orelse {
                std.log.warn("config: {s}:{}: invalid renderer '{s}' (expect d3d11, d3d12, opengl, pure-opengl, vulkan, or native-vulkan)", .{ source_name, line_no, value });
                continue;
            };
...
test "research renderers require exact explicit names" {
    ...
    inline for (.{ RendererBackend.d3d12, RendererBackend.opengl, RendererBackend.@"pure-opengl", RendererBackend.vulkan, RendererBackend.@"native-vulkan" }) |backend| {
    ...
    for ([_][]const u8{
        ...
        "renderer = native_vulkan\n",
        "renderer = nativevulkan\n",
        "renderer =\n",
    }) |src| {
        var cfg = parse(std.testing.allocator, src, "test");
        defer cfg.deinit();
...
test "native presentation backends request a normally redirected window" {
    try std.testing.expect(RendererBackend.@"pure-opengl".usesDwmRedirection());
    try std.testing.expect(RendererBackend.@"native-vulkan".usesDwmRedirection());
