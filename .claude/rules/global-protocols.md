...
If a tool returns `SESSION_ID`, record it and decide whether to continue the conversation in subsequent calls. Codex/AntiGravity sessions may be interrupted; continue the same session if needed response was not received.

## Sandbox Safety
Codex/AntiGravity must never write to the filesystem. Request `unified diff patch` output only.
